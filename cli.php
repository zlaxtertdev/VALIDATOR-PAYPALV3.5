 ...
 */
 ...
 reqemail:
 echo "$WH [$GR+$WH] Threads ($YL Max 15 $WH) ($YL Recommended 5-15 $WH) $GR>> $BL";
 ...
 foreach($lists as $list){
    ...
    if (strtolower($mode_proxy) == "off") {
     ...
     //API
     $api = $APIs."/validator/paypalV2/?list=$email&proxy=$Proxies&proxyAuth=$proxy_Auth&type_proxy=$type_proxy&apikey=$apikey";
     //CURL
 ...
 $rollingCurl->setCallback(function(\RollingCurl\Request $request, \RollingCurl\RollingCurl $rollingCurl) use (&$results) {
     ...
     if(strpos($x, '"status":"live"')){
        ...
        $code_country = $js['data']['code_country'];
        $device = $js['data']['device'];
        $app_Auth = $js['data']['app_Auth'];

        $live++;
        save_file("result/live.txt","$list($country)($device)");
        echo "[$RD$no$DEF/$GR$total$DEF][$CY$jam$DEF]$GR LIVE$DEF =>$BL $list$DEF | [$YL COUNTRY$DEF: $WH$country$DEF ] [$YL DEVICE$DEF: $WH$device$DEF ] [$YL MSG$DEF: $WH$msg$DEF ] | BY$CY DARKXCODE$DEF (V3.5)".PHP_EOL;
        ...
        echo "[$RD$no$DEF/$GR$total$DEF][$CY$jam$DEF]$RD BAD REQUEST$DEF =>$BL $list$DEF | [$YL MSG$DEF:$MG BAD REQUESTS!$DEF ] | BY$CY DARKXCODE$DEF (V3.5)".PHP_EOL;
 ...
 echo "[!] NOTE : CHECK AGAIN FILE 'unknown.txt' or 'RTO.txt' or 'bad-req.txt' [!]".PHP_EOL;
